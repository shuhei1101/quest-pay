...
- UserInfo: ユーザー情報表示
- PageTitle: ページタイトル（Paperでラップ、ボーダー・影付き、右側にアクション配置可能）
...
- LoadingIndicator: 画面右上のローディング表示（フローティング）
...
### PageTitle
**パス:** `app/(core)/_components/PageTitle.tsx`
**用途:** 各画面のページタイトルを統一的に表示
...
- 戻るボタンをデフォルトで表示（showBackButton=true）
...
**用途:** 画面右上にフローティング表示されるローディングインジケーター
...
### ページタイトルの統一
すべての主要画面でPageTitleを使用することで、UIの一貫性を保つ。
