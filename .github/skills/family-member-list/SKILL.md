...
このスキルは、家族メンバー一覧画面（親・子供）の2ペイン設計、データフェッチング、リスト操作の知識を提供します。PC時は2ペイン表示、モバイル時は一覧と詳細を切り替える設計です。

## メインソースファイル

### 画面コンポーネント
- `packages/web/app/(app)/families/members/page.tsx`: ルート（親のみアクセス可）
- `packages/web/app/(app)/families/members/layout.tsx`: 2ペインレイアウト実装
- `packages/web/app/(app)/families/members/FamilyMemberListScreen.tsx`: 一覧画面（空実装）

### 関連コンポーネント
- `packages/web/app/(app)/families/members/_components/FamilyMemberList.tsx`: メンバー一覧表示
- `packages/web/app/(app)/children/_components/ChildCardLayout.tsx`: 子供カード
- `packages/web/app/(app)/parents/_components/ParentCardLayout.tsx`: 親カード

### フック
- `packages/web/app/(app)/children/_hook/useChildren.ts`: 子供データ取得
- `packages/web/app/(app)/parents/_hook/useParents.ts`: 親データ取得
...
### コンポーネント構造を把握する場合
2ペイン設計、コンポーネント階層、スタイリングを確認：
```
references/component_structure.md
```

### データ取得方法を理解する場合
並列フェッチング、questStats取得、キャッシュ戦略を確認：
```
references/data_fetching.md
```

### 画面フローを追う場合
2ペイン切り替え、リストレンダリング、FAB制御を確認：
```
references/flow_diagram.md
```

### リスト操作を確認する場合
メンバー選択、FAB配置、選択状態管理を確認：
```
...
1. **全体像の把握**: `references/component_structure.md`で2ペイン設計確認
2. **データフローの理解**: `references/data_fetching.md`で並列取得パターン確認
3. **実装時**: `references/flow_diagram.md`で切り替えロジック確認
4. **操作実装時**: `references/list_operations.md`で選択・FAB制御確認
