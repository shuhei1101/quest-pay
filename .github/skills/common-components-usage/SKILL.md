...
### PageHeader
ページのヘッダーを表示する場合に使用。Paperでラップされ、ボーダーと影が自動的に付く。また、ログインユーザーに応じてプロフィールアイコンボタンが自動表示される（丸枠で表示）。
...
// プロフィールアイコンボタンが自動的に表示される
// 親の場合：家族のアイコン（家族プロフィール画面へ遷移）
// 子供の場合：子供のアイコン（子供プロフィール画面へ遷移）
...
// プロフィールアイコンボタンとアクションボタンの両方が表示される
<PageHeader 
  title="定額報酬"
  rightSection={
    <Button 
...
**アイコン表示について:**
- Avatarコンポーネントを使用して丸枠で表示
- グラデーション背景（紫系）が自動適用
- RenderIconコンポーネントで実際のユーザーアイコンを表示
- アイコンは白色で表示される
...
**設計方針:**
- すべての主要画面で統一的に使用することでUIの一貫性を保つ
- マージンボトムが自動設定されるため、後続要素との間隔を考慮不要
- 右側にアクションボタンを配置する場合、`variant="light"`または`variant="outline"`を推奨
...
### SubMenuFAB
サブメニューを表示する場合に使用。右下に固定され、上に展開される。
