...
このスキルは、子供ユーザーが受注したクエストの詳細を表示し、完了報告を行う画面の知識を提供します。
...
- `app/(app)/quests/child/_components/QuestStatusBadge.tsx`: ステータスバッジ

### フック
- `app/(app)/quests/child/_hooks/useChildQuest.ts`: 子供クエストデータ取得フック
- `app/(app)/quests/child/_hooks/useReportCompletion.ts`: 完了報告フック
- `app/(app)/quests/child/_hooks/useCancelReport.ts`: 報告キャンセルフック

### API（child-quest-apiスキル参照）
- `GET /api/quests/child/[id]`: 子供クエスト詳細取得
- `POST /api/quests/child/[id]/report`: 完了報告
- `POST /api/quests/child/[id]/cancel-report`: 報告キャンセル
...
### 3. アクション機能
- 完了報告（in_progress時）
- 報告キャンセル（pending_review時）
- 確認モーダル表示

## Reference Files Usage

### コンポーネント構造を把握する場合
画面レイアウト、使用コンポーネントの階層を確認：
```
references/component_structure.md
```

### データ表示ロジックを理解する場合
データ取得、変換、プレゼンテーション処理を確認：
```
references/data_display.md
```

### 画面フローを理解する場合
初期表示、完了報告、キャンセルのフローを確認：
```
...
3. **実装時**: `references/action_components.md`でアクション機能確認
