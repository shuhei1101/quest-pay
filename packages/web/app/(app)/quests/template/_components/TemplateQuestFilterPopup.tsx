...
import type { TemplateQuestFilterType } from "@/app/api/quests/template/query"
import { ActionIcon, Button, ColorPicker, Input, Modal, Pill, PillsInput, Popover, Space, Tabs, Text } from "@mantine/core"
import { Dispatch, SetStateAction, useEffect, useState } from "react"

/** テンプレートクエストフィルターポップアップ */
export const TemplateQuestFilterPopup = ({opened, close, currentFilter, handleSearch}: {
  opened: boolean,
  close: () => void,
  currentFilter: TemplateQuestFilterType,
  handleSearch: (filter: TemplateQuestFilterType) => void
}) => {
  /** クエストフィルター状態 */
  const [filter, setFilter] = useState<TemplateQuestFilterType>({tags: []})
    ...
    <Modal opened={opened} onClose={close} title="フィルター">
          ...
          <div className="flex justify-end gap-2">
            <Button variant="default" onClick={() => {
              const emptyFilter: TemplateQuestFilterType = {tags: []}
