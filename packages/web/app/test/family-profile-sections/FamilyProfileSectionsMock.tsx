...
import { Box, Card, Group, Stack, Text, Badge, ThemeIcon, SimpleGrid, Divider, Button, Anchor } from "@mantine/core"
import { useState } from "react"
import { PageHeader } from "@/app/(core)/_components/PageHeader"
import { RenderIcon } from "@/app/(app)/icons/_components/RenderIcon"
import { formatDate } from "@/app/(core)/util"
import { IconHome, IconCalendar, IconId, IconUser, IconBabyCarriage, IconTrophy, IconChecklist, IconStar, IconCoin, IconGift, IconArrowRight } from "@tabler/icons-react"
...
/** パターン3: 縦スクロール＋セクション方式 */
export const FamilyProfileSectionsMock = () => {
  ...
  return (
    <Box p="md">
      <PageHeader title="家族プロフィール（縦スクロール方式）" showProfileButton={false} />
      ...
      <Card shadow="sm" padding="lg" radius="md" withBorder mb="md">
        <Group justify="space-between" mb="md">
          <Group gap="xs">
            ...
            <Text size="lg" fw={600}>公開クエスト一覧</Text>
          </Group>
          <Anchor size="sm" c="violet">
            もっと見る <IconArrowRight size={14} style={{ verticalAlign: "middle" }} />
          </Anchor>
        </Group>
        <Stack gap="md">
      ...
      <Card shadow="sm" padding="lg" radius="md" withBorder mb="md">
        <Group justify="space-between" mb="md">
          <Group gap="xs">
            <IconCoin size={24} color="#ff8c00" />
            <Text size="lg" fw={600}>報酬テンプレート</Text>
          </Group>
          <Anchor size="sm" c="orange">
            もっと見る <IconArrowRight size={14} style={{ verticalAlign: "middle" }} />
          </Anchor>
        </Group>
        <Stack gap="md">
