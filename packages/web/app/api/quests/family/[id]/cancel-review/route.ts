...
export async function PUT(
  ...
  return withRouteErrorHandling(async () => {
    logger.info('クエスト完了報告キャンセルAPI開始', {
      path: '/api/quests/family/[id]/cancel-review',
      method: 'PUT',
    })
    ...
    logger.debug('完了報告キャンセル完了', { familyQuestId: id, childId: userInfo.children.id })
    
    logger.info('クエスト完了報告キャンセル成功', { familyQuestId: id })
