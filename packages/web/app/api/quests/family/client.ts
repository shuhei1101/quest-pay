import queryString from "query-string"
import { FAMILY_QUESTS_API_URL } from "@/app/(core)/endpoints";
import { AppError } from "@/app/(core)/error/appError";
import type { GetFamilyQuestsResponse, PostFamilyQuestRequest, PostFamilyQuestResponse } from "./route";
import { FamilyQuestSearchParams } from "./query";
import { logger } from "@/app/(core)/logger";

/** 家族クエストをGETする */
export const getFamilyQuests = async (params: FamilyQuestSearchParams) => {
  logger.debug('家族クエスト一覧API呼び出し開始', {
    url: FAMILY_QUESTS_API_URL,
    params,
  })
  
  try {
    // クエリストリングを生成する
    const qs = queryString.stringify(params, { arrayFormat: "none" })
    
    // APIを実行する
    const res = await fetch(`${FAMILY_QUESTS_API_URL}?${qs}`, {
      method: "GET",
      headers: { "Content-Type": "application/json" },
    })
    const data = await res.json()

    logger.debug('家族クエスト一覧APIレスポンス受信', {
      status: res.status,
      ok: res.ok,
    })

    // ステータスが不正な場合、アプリ例外を発生させる
    if (!res.ok) {
      logger.error('家族クエスト一覧APIエラー', {
        status: res.status,
        errorData: data,
      })
      throw AppError.fromResponse(data, res.status)
    }

    logger.debug('家族クエスト一覧取得完了', {
      questsCount: data.rows?.length ?? 0,
      totalRecords: data.totalRecords,
    })

    return data as GetFamilyQuestsResponse
  } catch (error) {
    logger.error('家族クエスト一覧取得中に例外発生', {
      error: error instanceof Error ? error.message : String(error),
    })
    throw error
  }
}

/** 家族クエストをPOSTする */
export const postFamilyQuest = async (request: PostFamilyQuestRequest) => {
  logger.debug('家族クエスト作成API呼び出し開始', {
    url: FAMILY_QUESTS_API_URL,
    questName: request.form.name,
  })
  
  try {
    // APIを実行する
    const res = await fetch(`${FAMILY_QUESTS_API_URL}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(request)
    })

    const data = await res.json()
    
    logger.debug('家族クエスト作成APIレスポンス受信', {
      status: res.status,
      ok: res.ok,
    })
    
    // ステータスが不正な場合、アプリ例外を発生させる
    if (!res.ok) {
      logger.error('家族クエスト作成APIエラー', {
        status: res.status,
        errorData: data,
      })
      throw AppError.fromResponse(data, res.status)
    }

    logger.debug('家族クエスト作成完了', {
      questId: data.questId,
    })

    return data as PostFamilyQuestResponse
  } catch (error) {
    logger.error('家族クエスト作成中に例外発生', {
      error: error instanceof Error ? error.message : String(error),
    })
    throw error
  }
}
