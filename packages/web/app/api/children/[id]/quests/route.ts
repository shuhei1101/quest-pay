...
export async function GET(
  ...
  return withRouteErrorHandling(async () => {
    logger.info('子供クエスト一覧取得API開始', {
      path: '/api/children/[id]/quests',
      method: 'GET',
    })
    ...
    logger.debug('クエリパラメーター取得完了', { params })
    ...
    if (userInfo.profiles.type === "parent") {
      ...
      if (child?.profiles?.familyId !== userInfo.profiles.familyId) {
      ...
      if (userInfo.children.id !== childId) {
        logger.warn('他の子供アクセス検出', {
          loginChildId: userInfo.children.id,
          accessChildId: childId,
        })
        throw new ServerError("他の子供のデータにアクセスしました。")
      }
    }

    // クエストを取得する
    const result = await fetchChildQuests({db, childId, params, familyId: userInfo.profiles.familyId})
    logger.debug('子供クエスト取得完了', {
      rowsCount: result.rows?.length ?? 0,
      totalRecords: result.totalRecords,
    })

    logger.info('子供クエスト一覧取得成功', {
      childId,
      questsCount: result.rows?.length ?? 0,
    })
